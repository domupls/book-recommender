...
def prepare_thumbnails(df: pd.DataFrame) -> pd.DataFrame:
    df['large_thumbnail'] = df['thumbnail'] + "?fife=w800"
    df['large_thumbnail'] = np.where(df['large_thumbnail'].isna(), "data/cover-not-found.jpg",
                                        df['large_thumbnail'])
    return df

books = pd.read_csv("data/books_with_emotions.csv")
books = prepare_thumbnails(books)
...
def get_vector_database() -> Chroma:
    ...
    if not os.path.exists(db_name):
        ...
        db_books = Chroma.from_documents(docs, embedding=hf_embeddings, persist_directory=db_name)
    else:
        db_books = Chroma(persist_directory=db_name, embedding_function=hf_embeddings)
    return db_books
...
def retrieve_semantic_recommendations(query: str, category: str = None, tone: str = None,
    ...
    # get books similar to the query
    database = get_vector_database()
    recs = database.similarity_search(query, k=initial_top_k)
    books_list = [int(rec.page_content.strip('"').split()[0]) for rec in recs]
    book_recs = books[books["isbn13"].isin(books_list)].head(final_top_k)

    # filter by category and tone if provided
    if category != "All":
...
def recommend_books(query: str, category: str = "All", tone: str = "Happy"):
...
with gr.Blocks(theme = gr.themes.Citrus()) as dashboard:
    ...
    output = gr.Gallery(columns = 8, rows = 2, show_label=False)
